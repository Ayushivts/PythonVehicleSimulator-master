...
def main():
    ...
    # STEP 2: Add simulation time
    df["time"] = simTime
    
    # STEP 3: Heading
    df["heading"] = np.degrees(df["yaw"])
    ...
    # STEP 5: INS drift
    drift_x = np.cumsum(np.random.normal(0, 0.02, len(df)))
    drift_y = np.cumsum(np.random.normal(0, 0.02, len(df)))
    
    df["ins_x"] = df["x"] + drift_x
    df["ins_y"] = df["y"] + drift_y
    
    # STEP 6: Water current
    df["current_x"] = np.random.normal(0, 0.1, len(df))
    df["current_y"] = np.random.normal(0, 0.1, len(df))
    ...
    df["az"] = np.gradient(df["w"], dt)
    
    # STEP 8: Save CSV
    df.to_csv(
        "maritime_navigation_dataset.csv",
        index=False
    )
    ...
    df.to_csv("raw_simulator_data.csv", index=False)
